/* svgs for theme toggle nutton */
let TO_DARK_THEME_ICON = 'data:image/svg+xml;base64,'
let TO_LIGHT_THEME_ICON = 'data:image/svg+xml;base64,'


/* Inclide Header */

jQuery(document).ready(function(){ 
						   
	 jQuery("header").load("./ajax/custom-header.html");
	 jQuery("div").css("border", "3px solid red");
	
});


/* DOM is ready; fires BEFORE window.onload */
document.onreadystatechange = function(e) {
    if (document.readyState === 'complete') {

        let current_theme = get_theme()
        let theme_icon = document.querySelector('img')
        if (current_theme === null) {
            current_theme = 'theme-light'
        }
        set_theme(theme_icon, current_theme)
    }
};

/* once all the content on the page is loaded */
window.onload = () => {

    if (document.getElementById('learn-more')) {
        document.getElementById('learn-more').addEventListener('click', toLearnMore)
    }
}

/* cahnges theme */
function change_theme() {
    let current_theme = get_theme()
    let theme_icon = document.querySelector('img')
    console.log(theme_icon)

    if (current_theme === 'theme-light') {
        set_theme(theme_icon, 'theme-dark')
    } else if (current_theme === 'theme-dark') {
        set_theme(theme_icon, 'theme-light')
    }
}

function get_theme() {
    return window.localStorage.getItem('theme')
}

function set_theme(theme_icon, theme) {
    window.localStorage.setItem('theme', theme)
    document.querySelector('body').className = theme;
    toggle_icon(theme_icon, theme)
}

function toggle_icon(theme_icon, theme) {
    if (theme === 'theme-dark') { theme_icon.setAttribute('src', TO_LIGHT_THEME_ICON); } else if (theme === 'theme-light') { theme_icon.setAttribute('src', TO_DARK_THEME_ICON); }
}

/* appends elements */
function toLearnMore(e) {

    /* creating more text about myself*/
    let moreInfoPar = document.createElement('p')
    moreInfoPar.innerHTML = 'I absolutely love the areas I study; although I have a bunch of other interests as well. Some of my hobbies include reading (mostly scientific literature, textbooks, popular science, classic russian literature, and poetry), photography, drawing, long-distance walking, knitting, crocheting, and gardening.'
    document.getElementById('more-info').appendChild(moreInfoPar)
    document.getElementById('learn-more').remove()
}

/* Titles */
window.onblur = function() { document.title = 'aww, come back!'; }
window.onfocus = function() { document.title = 'anastasia'; }

